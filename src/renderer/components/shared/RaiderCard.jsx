...
export default function RaiderCard({ raider, onOpenNotes, activeSession, onReplaceTrigger, onSelectRaider }) {
    ...
    if (highs > 0) {
        ...
        alertBadge = <span className="bg-tokyo-orange/15 text-tokyo-orange border border-tokyo-orange/40 text-[9px] px-1 rounded font-mono font-bold uppercase tracking-wider">Alerta</span>;
    } else if (lows > 0) {
    ...
    return (
        <div className={`rounded bg-liner-to-br ${classStyle.bg} flex flex-col justify-between p-2.5 h-full min-h-21.25 border border-tokyo-panel border-l-4 ${alertBorder} shadow-md group relative select-none`}>
            {activeSession && onReplaceTrigger && (
                <button
                    ...
                    className="absolute -top-1.5 -right-1.5 opacity-0 group-hover:opacity-100 bg-tokyo-purple hover:bg-[#ff007f] text-tokyo-main text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center shadow-lg transition-all cursor-pointer z-30 transform hover:scale-110"
                >
                    🔄
                </button>
            )}
            ...
            <div>
                ...
                <div className="flex justify-between items-center mt-1">
                    <span className="text-[9px] uppercase text-tokyo-comment tracking-widest font-semibold font-mono">
                        {raider.class}
                    </span>
                    ...
                    <button
                        ...
                        className="text-[9px] bg-[#1f2335] border border-tokyo-border/60 text-gray-400 hover:text-white hover:border-[#7dcfff] px-1.5 py-0.5 rounded font-mono transition-all opacity-40 group-hover:opacity-100 z-10 relative"
                    >
                        📝 NOTA
                    </button>
                </div>
            ...
            <button
                ...
                className="flex justify-between items-center mt-1.5 pt-1 border-t border-tokyo-panel/40 text-[10px] font-mono text-tokyo-comment w-full text-left hover:text-slate-300 transition-colors cursor-pointer"
            >
                <div className="flex gap-1.5">
                    <span>L:<span className={lows > 0 ? 'text-[#7dcfff] font-bold' : ''}>{lows}</span></span>
                    <span>M:<span className={mediums > 0 ? 'text-tokyo-orange font-bold' : ''}>{mediums}</span></span>
                    <span>H:<span className={highs > 0 ? 'text-[#c41f3b] font-bold' : ''}>{highs}</span></span>
                </div>
                {gravityTotal > 0 && (
                    <span className="text-[9px] bg-tokyo-panel px-1 rounded text-gray-400">
                        {gravityTotal} pts
                    </span>
                )}
            </button>
